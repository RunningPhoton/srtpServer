...
import com.demo.tools.FileHelp;
...
public class UserAction extends ActionSupport {
	...
	public String listWhoIAdd() {
		try {
			TDemoUser tempUser = iuserService.findByUserName(this.user
					.getUserName());
	...
	public String listWhoAddMe() {
		try {
			TDemoUser tempUser = iuserService.findByUserName(this.user
					.getUserName());
			this.many = iuserService.listUserSets(tempUser
					.getUserBeenFriendSet());
			return "many";
		} catch (Exception e) {
			// System.out.println(e);
			return ERROR;
	...
	public String upload() throws Exception {
		...
		if (!"".equals(file)) {
			...
			// 调用上传
			String pname = FileHelp.upload(file2, file2.getName());
			System.out.println("pname = " + pname);
			this.operObject = "http://120.77.171.222:8080/srtp_server/" + pname;

			System.out.println("photo name is " + this.operObject);
			return this.setGraphName();
		}
		return ERROR;
	}
	...
	public String setGraphName() {
		...
		if (iuserService.update(this.user)) {
			try {
				...
				if (tool.refresh(user.getUserId().toString(),
						user.getUserName(), user.getGraphName()) == true) {
	...
	public String listCircleMessage() {
		try {
			...
			while (iter.hasNext()) {
				...
				obj.add(new TDemoImplMessage(cur.getUser().getUserName(), cur
						.getCircleMessageContent(), new Date(cur.getOpertime()
						.getTime())));
	...
	public String createCircle() {
		try {
			this.user = iuserService.findByUserName(this.user.getUserName());
			if (iuserService.createCircle(this.user.getUserId().toString(),
					this.operObject, this.operContent)) {
				this.setCircle();
